...
│  ├─ effects.ts          파편 · 충격파 · 점수 팝업 · 화면 흔들림 (연출 전용)
...
**타격감** — 벽돌이 깨질 때 여섯 가지가 동시에 터집니다. 파편 14조각(공이 온 방향으로 쏠림),
충격파 링, 화면 흔들림, **히트스톱**(35~55ms 동안 공의 물리만 멈춤 — 파편은 계속 날아가므로
"박혔다 튕긴다"는 느낌이 난다), 저음+화이트노이즈를 겹친 파열음, 점수 팝업입니다.
연출은 [effects.ts](src/game/effects.ts)에 격리되어 있어 여기서 무엇이 몇 개 생기든
점수·충돌·판정은 달라지지 않습니다. 파편은 240개로 상한을 두고, OS의 "동작 줄이기"가 켜져 있으면
화면 흔들림만 자동으로 꺼집니다.
